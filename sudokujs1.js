...
function NumberBox(boxs, pw, pk, wsp_y, wsp_x) {
  var z, x, zmf;
  zmf = false;
...
function BeginFromCell(tab, obj) {
...
function sleep(objtime) {
  objtime.date = new Date().getSeconds();
  do {
    objtime.currentDate = new Date().getSeconds();
  } while (objtime.currentDate === objtime.date);
}

function myFunc(ms) {
  setTimeout(
   function(){
     if (ms % 6 === 0) message = "Checking "
     else message += ". ";
...
var memorydiffrence = -1; */

function SudokuSolution(tab_kwad, opcje, pkw, pkk, rozpatryliczbakratek, obj, second1, additionalvar, wariant, dot) {
  var
  ...
  // else if (memorydiffrence !== diffrence){ memorydiffrence = diffrence; myFunc(diffrence); }
   ...
   if (obj.zmiennastop === false){
	for (cyfra = 1; cyfra <= 9; cyfra++) {
      ...
      if ((SprWieKol(tab_kwad) === false) && (NumberBox(tab_kwad, pkw, pkk, wsp_y, wsp_x) === false)) {
        if (rozpatryliczbakratek === obj1.max) opcje = opcje + 1;
        if (rozpatryliczbakratek === 81) rozpatryliczbakratek = 0;
        SudokuSolution(tab_kwad, opcje, pkw, pkk, rozpatryliczbakratek + 1, obj, second1, additionalvar, wariant, dot);
...
function checkdatainput(smallbox) {
  ...
  for (var checkindeks = 1; checkindeks <= 81; checkindeks++) {
    ...
    if (NumberBox(smallbox, beginrows, begincols, coordinates_y, coordinates_x) === true) {
      valuefunction = false;
      break;
    }
  }
...
function CreatNewBoard(mainboard,LengthMainBoard,obj,secondindex){
  ...
  SudokuSolution(mainboard, 0, 1, 1, 1, obj, 0, 1, 1);
